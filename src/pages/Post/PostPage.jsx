// src/app/pages/Post/PostPage.jsx
...
const MarkdownEditor = ({
  ...
  const insertCodeBlock = (lang = "") => {
    ...
    view.dispatch({
      ...
      selection: {
        anchor: cursor,
        head: cursor + (sel.empty ? 0 : text.length),
      },
    });
  };
        ...
        <div className={styles.editorColumn}>
          ...
          <div className={styles.editorToolbar}>
            <Button.Group size="mini">
              <Button type="button" basic onClick={() => toggleHeading(1)}>
                H1
              </Button>
              <Button type="button" basic onClick={() => toggleHeading(2)}>
                H2
              </Button>
              <Button type="button" basic onClick={() => toggleHeading(3)}>
                H3
              </Button>
            </Button.Group>
            <Button.Group size="mini">
              <Button type="button" basic onClick={() => wrapSelection("**")}>
                Bold
              </Button>
              <Button type="button" basic onClick={() => wrapSelection("_")}>
                Italic
              </Button>
              <Button type="button" basic onClick={() => wrapSelection("`")}>
                Code
...
const TagInput = ({
  ...
  return (
    <Form.Field required={required}>
      ...
      <small>
        {tags.length} tag{tags.length === 1 ? "" : "s"} added. Use Enter / “,” /
        Tab to add.
      </small>
      {hint && (
...
const PostPage = () => {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `article-images/${uid}/${Date.now()}-${safe}`
      );
      const metadata = {
  ...
  const handleSubmit = async () => {
    ...
    const activeTitle = isArticleMode ? articleTitle : questionTitle;
    if (!activeTitle.trim()) return setFormErr("Title is required.");
  ...
  const tags = isArticle ? articleTags : qTags;

  const currentTitle = isArticle ? articleTitle : questionTitle;
  const currentBody = isArticle ? body : qDescription;

  const handleTitleChange = (next) => {
    if (isArticle) setArticleTitle(next);
    else setQuestionTitle(next);
  };

  const replaceBody = (next) => {
    if (isArticle) setBody(next);
    else setQDescription(next);
  };

  const appendBody = (snippet) => {
    if (!snippet) return;
    if (isArticle) {
      setBody((prev) => (prev ? `${prev}\n\n${snippet}` : snippet));
    } else {
      setQDescription((prev) => (prev ? `${prev}\n\n${snippet}` : snippet));
    }
  };

  // Snapshot of the current editing context for AI Editor
  const contextSnapshot = () => {
    const tagLine = isArticle
      ? (articleTags || []).join(", ")
      : (qTags || []).join(", ");
    const bodySrc = isArticle ? body : qDescription;
    return [
      `postType=${postType}`,
      `#TITLE\n${currentTitle || ""}`,
      `#ABSTRACT\n${abstract || ""}`,
      `#BODY_SNIPPET\n${(bodySrc || "").slice(0, 4000)}`,
      `#TAGS\n${tagLine}`,
      "",
      "When possible, reply using any relevant sections:",
      "#TITLE",
      "#ABSTRACT",
      "#BODY_MD",
      "#TAGS",
    ...
    <div className={styles.container}>
                ...
                {imageUrl && (
                  <div className={styles.uploadPreviewBlock}>
                    ...
                    <Message
                      positive
                      size="tiny"
                      className={styles.uploadSuccessMsg}
                    >
                      Image uploaded successfully.
                    </Message>
                    <span onClick={clearImage} className={styles.clearImage}>
      ...
      {isArticle && (
        <AIEditor
          ...
          onApplyTitle={setArticleTitle}
          onApplyAbstract={setAbstract}
          onReplaceBody={replaceBody}
          onAppendBody={appendBody}
          onApplyTags={setArticleTags}
        />
      )}
    </div>
  );
