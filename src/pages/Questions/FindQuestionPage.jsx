...
const FindQuestionPage = () => {
  ...
  const pendingRaw = search !== deferredSearch;
  const searchPending = useDelayedFlag(pendingRaw, 150);
  ...
  const totalFiltered = filteredVisible.length;
  const visibleQuestions = useMemo(
    () => filteredVisible.slice(0, Math.min(visibleCount, totalFiltered)),
    [filteredVisible, totalFiltered, visibleCount]
  );
  const hasMoreVisible = visibleCount < totalFiltered;

  const filterSignature = useMemo(() => {
    const range = dateRange.ranges[0] || {};
    const start = range.startDate?.toISOString?.() ?? "";
    const end = range.endDate?.toISOString?.() ?? "";
    return [deferredSearch.trim().toLowerCase(), selectedTags.join("|"), dateRange.active ? "1" : "0", start, end].join("::");
  }, [dateRange, deferredSearch, selectedTags]);
  ...
  useEffect(() => {
    if (!hasMoreVisible) return undefined;
      ...
      (entries) => {
        ...
        if (first?.isIntersecting) {
          setVisibleCount((prev) => {
            ...
            return Math.min(prev + PAGE_SIZE, totalFiltered);
          });
        }
      },
      { rootMargin: "200px" }
  ...
  const onDragEnd = ({ active, over }) => {
    ...
    const visIds = visibleQuestions.map((q) => q.id);
